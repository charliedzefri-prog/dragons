...
function applySkill(actor,k,target,atkM,blk){
  ...
  if(s.type==="attack"){
    ...
    targets.forEach(t=>{const m=elMult(s.el,actor,t);const crit=RNG()<(actor.crit||0.1);const vuln=(t.fx||[]).some(e=>e.k==="vuln")?1.4:1;
      ...
      dmg=Math.max(1,Math.round(dmg));if(s.execute)dmg=Math.max(dmg,Math.round(t.maxhp*s.execute));if(t.hpScale&&t.hpScale!==1)dmg=Math.round(dmg*t.hpScale*(MOOSE_EVENT.dmgTaken||1));if(t.soviet&&t.side==="enemy")dmg=Math.max(1,Math.round(dmg*(SOVIET_EVENT.dmgTaken||1)));
...
function charaMission(id){const m=CHARA_MISSIONS.find(x=>x.id===id);if(!m)return;closeModal();charaWalk(m,()=>{
  const lvl=m.lvl;
  startBattle({name:"Чара — "+m.name,lvl,ids:m.ids,bg:"nightmare0",boss:!!m.boss,allies:["d163"],allyOv:{lvl:Math.min(50,lvl+8),stars:5},music:"battle",onEnd:(win)=>{
    ...
    else{modal(`<div class="result lose" style="font-family:monospace">* Ты умерла. Но ты полна решимости.</div><div class="row" style="justify-content:center"><button class="btn" onclick="closeModal();show('events')">Попробовать снова</button></div>`,{closable:false})}}})})}
