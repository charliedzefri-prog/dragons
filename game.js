...
function applySkill(actor,k,target,atkM,blk){
  ...
  if(s.type==="attack"){
    ...
    targets.forEach(t=>{const m=elMult(s.el,actor,t);const crit=RNG()<(actor.crit||0.1);const vuln=(t.fx||[]).some(e=>e.k==="vuln")?1.4:1;
      ...
      dmg=Math.max(1,Math.round(dmg));if(s.execute)dmg=Math.max(dmg,Math.round(t.maxhp*s.execute));if(t.hpScale&&t.hpScale!==1)dmg=Math.round(dmg*t.hpScale*(MOOSE_EVENT.dmgTaken||1));
