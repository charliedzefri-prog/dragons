...
function charaCardHtml(){const c=charaState();if(!c.found)return "";const doneN=Object.keys(c.done).length;return `<div class="panel chara-card"><h2>🔪 ??? <span class="badge" style="background:#000">ур.15+</span></h2><div class="feature"><img src="assets/d163.png" class="johnny-img" style="image-rendering:pixelated"><div style="flex:1"><div class="desc">Кто-то другой хочет играть. Пройдено: ${doneN}/${CHARA_MISSIONS.length}.</div><div class="row">${CHARA_MISSIONS.map((m,i)=>`<button class="btn ${c.done[m.id]?"":"red"} sm" data-cm="${m.id}" ${i>0&&!c.done[CHARA_MISSIONS[i-1].id]?"disabled":""}>${c.done[m.id]?"✔ ":""}${m.name}</button>`).join("")}</div><small>Бои только за Чару. Игра выдаст её сама.</small></div></div></div>`}
function charaMission(id){const m=CHARA_MISSIONS.find(x=>x.id===id);if(!m)return;closeModal();charaWalk(m,()=>{
  const lvl=m.lvl;
  try{startBattle({name:"Чара — "+m.name,lvl,ids:m.ids,bg:"nightmare0",boss:!!m.boss,allies:["d163"],allyOv:{lvl:Math.min(50,lvl+8),stars:5},music:"battle",onEnd:(win)=>{
    if(win){const c=charaState();const first=!c.done[m.id];c.done[m.id]=1;const gold=2500*CHARA_MISSIONS.indexOf(m)+2500,gems=first?15:3;S.gold+=gold;S.gems+=gems;addXP(300);save();updateTop();
      modal(`<div class="result win" style="color:#f00;font-family:monospace">* ${m.win}</div><div class="center" style="font-weight:900">+🪙${fmt(gold)} +💎${gems}</div><div class="row" style="justify-content:center"><button class="btn" onclick="closeModal();show('events')">…</button></div>`,{closable:false})}
    else{modal(`<div class="result lose" style="font-family:monospace">* Ты умерла. Но ты полна решимости.</div><div class="row" style="justify-content:center"><button class="btn" onclick="closeModal();show('events')">Попробовать снова</button></div>`,{closable:false})}}})}catch(err){console.error(err);B=null;modal(`<h2 style="color:#f00;font-family:monospace">* Ошибка</h2><p style="font-family:monospace">${String(err&&err.message||err)}</p><p><small>${String(err&&err.stack||"").split("\n").slice(0,3).join("<br>")}</small></p><div class="row"><button class="btn" onclick="closeModal();show('events')">Ок</button></div>`)}})}
// мини-сцена: ходим Чарой по тёмной комнате (стрелки/WASD/свайп/джойстик), доходим до цели — начинается бой
function charaWalk(m,done){const root=$("#modal-root");root.innerHTML=`<div class="chara-ov"><div class="chara-room" id="croom"><div class="chara-txt">* ${m.intro}</div><div class="chara-goal" id="cgoal">✦</div><div class="chara-pl" id="cpl"></div><div class="chara-hint">стрелки / WASD / свайп — дойди до ✦ &nbsp; <a id="cskip">пропустить</a></div><div class="cpad"><button data-d="up">▲</button><div><button data-d="left">◀</button><button data-d="down">▼</button><button data-d="right">▶</button></div></div></div></div>`;
  ...
  goal.style.left=Math.max(200,W()-140)+"px";goal.style.top=Math.min(H()-60,Math.max(80,H()*0.45))+"px";y=Math.max(0,Math.min(H()-96,parseFloat(goal.style.top)-44));
  ...
  const step=()=>{if(stop)return;let dx=0,dy=0;if(keys.left)dx-=1;if(keys.right)dx+=1;if(keys.up)dy-=1;if(keys.down)dy+=1;moving=dx||dy;if(moving){if(dy>0)dir=0;else if(dy<0)dir=3;if(dx<0)dir=1;else if(dx>0)dir=2;x=Math.max(0,Math.min(W()-96,x+dx*3));y=Math.max(0,Math.min(H()-96,y+dy*3));if(++tick%8===0)frame=(frame+1)%3}else frame=1;draw();
