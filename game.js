...
function renderDragons(){
  ...
  const onq=e=>{S._q=e.target.value;const raw=(e.target.value||"").toLowerCase().replace(/[\s.]/g,"");if(raw===RORY_EVENT.code&&!S.roryFound){if(S.level<RORY_EVENT.req){toast("…снизу что-то мычит. ур."+RORY_EVENT.req);return}S.roryFound=true;save();e.target.blur();toast("🐂 Мууу. Под островом открылся люк.");setTimeout(()=>show("events"),500);return}if((raw==="чара"||raw==="chara")&&!charaState().found){if(S.level<15){toast("* …ещё не время. ур.15");return}charaState().found=true;save();e.target.blur();document.body.classList.add("hell-glitch");setTimeout(()=>{document.body.classList.remove("hell-glitch");toast("<b style=\"color:#f00;font-family:monospace\">* Приветик.</b>");show("events")},700);return}if(chk(e.target.value)&&!S.mooseFound){if(S.level<MOOSE_EVENT.req){toast("<b style=\"color:#f00\">…ещё рано. ур."+MOOSE_EVENT.req+"</b>");return}S.mooseFound=true;save();e.target.blur();mooseGlitch(1400);playMusic("drone");setTimeout(()=>{toast("<b style=\"color:#f00\">он проснулся</b>");show("events");playMusic("drone");$("#navbar").classList.add("locked")},1400);return}draw()};
...
function renderBattle(){
  ...
  root.innerHTML=`<div class="arena ${B.opp.horror?"horror falling":""} ${B.opp.johnny?"johnny":""} ${B.opp.johnny&&B.enemies.some(f=>f.id==="d157"&&f._phase2)?"phase2":""}">
...
function openRory(){const g=roryState();if(g.floor>=RORY_EVENT.floors){modal(`<h2>🐂 Подвал Бычков Рори</h2><p>Подвал пройден до конца. Рекорд: ${g.best}/${RORY_EVENT.floors}.</p><div class="row"><button class="btn" id="rr">Начать заново</button></div>`);$("#rr").onclick=()=>{g.floor=0;g.hp=null;save();closeModal();openRory()};return}
  ...
  const m=modal(`<h2>🐂 Подвал Бычков Рори <span class="badge gold">этаж ${f+1}/${RORY_EVENT.floors}</span></h2><div class="feature"><img src="assets/d35.png" style="height:120px;filter:brightness(.6) drop-shadow(0 0 12px #000)"><div style="flex:1"><div class="desc">Под островом что-то мычит. ${n} бычк${n===1?"ок":"а"} уровня <b>${lvl}</b>.${imm?" <b style='color:#f66'>БЕССМЕРТИЕ:</b> с этого этажа бычки не умирают от одного удара — их надо добивать.":""}<br><small>HP команды переносится между этажами. Поражение — на первый этаж.</small></div></div></div>
  ${g.hp?`<small>HP команды: ${Object.entries(g.hp).map(([id,h])=>dInfo(id).name+" "+h).join(", ")}</small>`:""}
  ...
  $("#rgo",m).onclick=()=>{closeModal();pickTeamModal("🐂 Этаж "+(f+1),g.hp?"<p>⚠️ HP команды переносится.</p>":"",()=>{
    startBattle({name:"Подвал Рори, этаж "+(f+1),lvl,bg:BATTLE_BG.dungeon,ids:Array(n).fill("d35"),carry:g.hp||null,rory:true,immortal:imm,boss:f+1===RORY_EVENT.floors,onEnd:(win,hpLeft)=>{
      ...
      else{g.floor=0;g.hp=null;save();modal(`<div class="result lose">МУУУ.</div><p class="center">Бычки затоптали. Рекорд: ${g.best}.</p><div class="row" style="justify-content:center"><button class="btn" onclick="closeModal();show('events')">Ок</button></div>`,{closable:false})}}})})};
