...
function renderFriends(){const root=$("#scr-friends");const on=NET.status==="online";const me=NET.me;
  ...
  $("#pname",root).onchange=e=>{S.profile.name=e.target.value.trim()||"Хранитель";save();updateTop();if(/^(чара|chara)$/i.test(S.profile.name)&&!charaState().found){if(S.level<15){toast("* …ещё не время.");}else{charaState().found=true;save();document.body.classList.add("hell-glitch");setTimeout(()=>{document.body.classList.remove("hell-glitch");toast("<b style=\"color:#f00;font-family:monospace\">* Это имя… Приветик.</b>");show("events")},900)}}netSend({t:"profile",name:S.profile.name,avatar:S.profile.avatar,power:team.reduce((a,id)=>a+power(id),0),team})};
