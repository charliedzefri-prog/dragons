...
function show(scr){
  ...
  if(S.mooseFound&&scr!=="events"&&scr!=="battle"&&!B){show("events");return}
...
function renderMap(){
  ...
  $$("[data-isl]",root).forEach(b=>b.onclick=()=>{const df=ISLANDS[+b.dataset.isl];const ci=S.islands.findIndex(x=>x.id===df.id);
    if(ci>=0){S.cur=ci;save();renderMap();return}
...
let moveFrom=null,moveIsl=null;
...
function tileClick(i){
  const t=S.tiles[i];
  if(moveFrom!==null){const srcIsl=S.islands[moveIsl==null?S.cur:moveIsl]||curIsland();const from=srcIsl.tiles[moveFrom];if(!from||!from.b){moveFrom=null;moveIsl=null;renderMap();return}
    if(i===moveFrom&&srcIsl===curIsland()){moveFrom=null;moveIsl=null;renderMap();return}
    if(from.b==="incubator"&&srcIsl!==curIsland()){toast("Инкубатор нельзя перенести на другой остров");return}
    if(!t.unlocked){toast("Участок закрыт");return}
    if(t.b){toast("Клетка занята — выбери пустую");return}
    Object.assign(t,{b:from.b,dragons:from.dragons,last:from.last,lvl:from.lvl});from.b=null;from.dragons=[];from.lvl=1;const cross=srcIsl!==curIsland();moveFrom=null;moveIsl=null;save();renderMap();toast(cross?"🚢 Здание перевезено на другой остров":"📦 Здание перемещено");return}
  ...
  if(b.special==="library"){const left=Math.max(0,LIBRARY.time-(Date.now()-t.last));const ready=left<=0;
    ...
    $("#mv",m).onclick=()=>{moveFrom=i;moveIsl=S.cur;closeModal();renderMap()};return}
  ...
  const m=modal(html);
  $("#mv",m).onclick=()=>{moveFrom=i;moveIsl=S.cur;closeModal();renderMap()};
...
function renderDragons(){
  ...
  const inp=$("#dsearch",root);inp.addEventListener("keyup",e=>{const v=(e.target.value||"").trim();if(v==="666"&&!S.ring666got){if(!S.biscuitTwist){toast("…кольцо ещё не выковано.");return}S.ring666got=true;save();e.target.value="";S._q="";jumpscare(700);setTimeout(()=>{modal(`<h2 style="color:#f00;font-family:monospace">💍 SECRET 666 RING</h2><div class="center"><div class="ring666">💍</div><p style="font-family:monospace;color:#f66">* Кольцо Бисквит. Три шестёрки на внутренней стороне.<br>* Пока оно у тебя: <b>+6.66% атаки и +6.66% крита</b> всей команде, а Бисквит… знает, где ты.</p></div><div class="row"><button class="btn red" onclick="closeModal()">Надеть</button></div>`)},900)}});inp.oninput=onq;inp.onchange=onq;inp.onkeyup=onq;inp.setAttribute("autocomplete","off");inp.setAttribute("autocorrect","off");inp.setAttribute("autocapitalize","off");inp.setAttribute("spellcheck","false");$$(".chip",root).forEach(b=>b.onclick=()=>{S._f=b.dataset.f;$$(".chip",root).forEach(x=>x.classList.toggle("on",x===b));draw()});draw();
}
function dragonDetail(id){
  ...
  <div class="dd-top"><img src="assets/${id}.png" id="ddimg"><div style="flex:1">
  ...
  if(id==="d35"&&!S.roryFound){const im=$("#ddimg",m);let n=0;im.style.cursor="pointer";im.onclick=()=>{n++;im.classList.add("hit");setTimeout(()=>im.classList.remove("hit"),300);sndPlay("moo.wav",.7);if(n<7){toast(["Мууу.","Му-у-у…","МУУУ!","…мычит куда-то вниз.","Из-под пола ответили.","Пол дрожит.","…"][n-1])}else{if(S.level<RORY_EVENT.req){toast("Под полом что-то огромное. Нужен ур."+RORY_EVENT.req);n=0;return}S.roryFound=true;save();closeModal();document.body.classList.add("hell-shake");setTimeout(()=>document.body.classList.remove("hell-shake"),800);toast("🐂 Под островом открылся люк. Смотри в Событиях.");setTimeout(()=>show("events"),900)}}}
...
let ov_rory=false;
function enemySkills(id,lvl){const d=dInfo(id);if(d.els.includes("soviet"))return ["sovhit","sovhit_s","sovban"];if(id==="d157")return ["staffslam","staffcut","doom","sunburst","sunrise"];if(id==="d160")return ["hellgaze","hellfire","hellswap","helldrain","hellheal","hellkara"];if(id==="d35"&&ov_rory)return ["bullrush","maul","basic"];if(id==="d164")return ["quake","stampede","roar","bullrush","hornwall"];const b=baseSkills(id).filter((k,i)=>lvl>=SLOT_LEVEL[i]);if(lvl>=8)b.push(ELEMENT_SPREAD[d.els[0]]);if(lvl>=12)b.push(ELEMENT_ABILITY[d.els[0]]);return b}
function mkFighter(id,ov,side,i){const st=dragonStats(id,ov);const d=dInfo(id);if(d.els.includes("soviet")){st.hp=Math.round(st.hp*(ov&&ov.enemy?(SOVIET_EVENT.hpMult||0.55):0.55))}let hpScale=1;if(id==="d160"&&ov&&ov.enemy){hpScale=MOOSE_EVENT.hp/st.hp;st.hp=MOOSE_EVENT.hp;st.def=Math.round(st.def*MOOSE_EVENT.defMult);st.atk=Math.round(st.atk*(MOOSE_EVENT.atkMult||1))}return {hpScale,immortal:!!(ov&&ov.immortal),id,side,i,name:d.name,els:d.els,skills:(ov&&ov.enemy)?enemySkills(id,ov.lvl):dSkills(id),hp:st.hp,maxhp:st.hp,atk:st.atk,def:st.def,spd:st.spd,cds:{},buffAtk:0,buffDef:0,debuff:0,crit:st.crit,timeb:st.timeb,lifesteal:st.lifesteal||0,dmgB:st.dmg||{},shield:Math.round(st.hp*(st.startshield||0)),fx:[],tb:st.tb||{},reviveUsed:false,soviet:d.els.includes("soviet"),charge:d.els.includes("soviet")?(side==="enemy"?-i:0):0,lvl:ov?ov.lvl:S.dragons[id].lvl}}
function startBattle(opp){
  if(team.length===0&&!opp.allies){toast("Выбери хотя бы одного дракона!");return}
  const ov={lvl:opp.lvl,stars:Math.min(5,1+Math.floor(opp.lvl/10)),enemy:true,immortal:!!opp.immortal};ov_rory=!!opp.rory;
  show("battle");
  RNG=opp.seed?seededRng(opp.seed):Math.random;
  B={opp,allies:(opp.allies||team).map((id,i)=>{const f=opp.allyOv?mkFighter(id,Object.assign({},opp.allyOv,{enemy:false}),"ally",i):mkFighter(id,null,"ally",i);if(opp.carry&&opp.carry[id]){f.hp=Math.min(f.maxhp,opp.carry[id])}return f}),enemies:opp.ids.map((id,i)=>mkFighter(id,ov,"enemy",i)),log:[],round:0,queue:[],busy:false,sel:null,pvp:!!opp.pvp,turnNo:0};
  ...
  if(S.ring666got&&!opp.pvp){B.allies.forEach(f=>{f.atk=Math.round(f.atk*1.0666);f.crit=(f.crit||0.1)+0.0666});setTimeout(()=>{const a=$(".arena-field");if(a){const d=document.createElement("div");d.className="ring-glow";d.textContent="💍 666";a.append(d);setTimeout(()=>d.remove(),2500)}},300)}
  if(opp.chara){const cb=window.__charaBuff||{};B.allies.forEach(f=>{if(cb.atk){f.atk=Math.round(f.atk*(1+cb.atk))}if(cb.heal){f.shield=(f.shield||0)+Math.round(f.maxhp*0.25)}});window.__charaBuff=null;setTimeout(()=>charaSay(),1200)}
...
function nextTurn(first){
  if(!B)return;
  {const j=B.enemies.find(f=>f.id==="d157"&&f.hp<=0&&!f._phase2&&B.opp.johnny);if(j){j._phase2=true;j.hp=Math.round(j.maxhp*.85);j.atk=Math.round(j.atk*1.6);j.def=Math.round(j.def*1.3);j.spd=Math.round(j.spd*1.4);j.fx=[];j.debuff=0;j.cds={};j._sunT=0;const ae=fEl(j);if(ae){ae.classList.remove("dead")}johnnyPhase2Fx();log("☀️ ВЕЛИКИЙ ДЖОННИ ПОДНИМАЕТ ПОСОХ: ВТОРАЯ ФАЗА");renderBattle();B.busy=true;setTimeout(()=>{B.busy=false;nextTurn()},2200);return}}
  ...
  f0.fx=(f0.fx||[]).filter(e=>{e.t--;
    ...
    if(e.k==="hot"&&(f0.fx||[]).some(x=>x.k==="noheal")){const h=Math.round(f0.maxhp*0.1);const boss=alive(f0.side==="ally"?B.enemies:B.allies).find(x=>x.id==="d160");if(boss){boss.hp=Math.min(boss.maxhp,boss.hp+h);log(`🚫 ПРИГОВОР: регенерация ${f0.name} ушла Лосю (+${h})`)}}
    ...
    return e.t>0});
  if(f0.soviet&&f0.hp>0&&skip){f0.charge=0;log(`🖥️ ${f0.name}: СБОЙ — загрузка сброшена (0/3)`);pop(fEl(f0),"СБОЙ 0/3","weak")}
...
function syncHellAnim(){if(!B||!B.opp.horror)return;const t=performance.now();const set=(el,durs)=>{if(el)el.style.animationDelay=durs.map(d=>(-(t%d))+"ms").join(",")};set($(".arena-field"),[1400,3000]);set($(".fighter.moose img"),[4000]);$$(".fighter.ally img").forEach((e,i)=>{e.style.animationDelay=(-(t%2600)-i*900)+"ms"})}
function renderBattle(){
  const root=$("#scr-battle");const f=B.cur;
  const fh=(x)=>`<div class="fighter ${x.side} ${dInfo(x.id).noflip?"noflip":""} ${dInfo(x.id).flipAlly?"flipally":""} ${dInfo(x.id).big?"bigspr":""} ${x.lvl>=50?"maxaura":""} ${x.id==="d160"?"moose":""} ${x.banned?"banned":""} ${x.hp<=0?"dead":""} ${x===f?"active":""} pos${x.i}" data-side="${x.side}" data-i="${x.i}"><img src="assets/${x.id==="d163"?"d163_battle":x.id}${x._phase2&&x.id==="d157"?"_raise":""}.png"><div class="hpbar"><div class="${x.hp/x.maxhp<.3?"low":""}" style="width:${x.hp/x.maxhp*100}%"></div>${x.shield?`<div class="shieldbar" style="width:${Math.min(100,x.shield/x.maxhp*100)}%"></div>`:""}</div>${x.soviet&&x.hp>0?`<div class="charge ${x.charge>=2?"hot":""}"><span style="width:${Math.min(100,(x.charge||0)/3*100)}%"></span><b>${x.charge>=3?"⚠ BANNED!":"⏳ "+Math.max(0,x.charge||0)+"/3"}</b></div>`:""}${x.banned?`<div class="banmark">BANNED</div>`:""}<div class="fn">${x.name} <small>ур.${x.lvl}</small></div><div>${x.els.map(elIco).join("")}</div>${x.buffAtk?"<small>⚔️↑</small>":""}${x.buffDef?"<small>🛡️↑</small>":""}${x.debuff?"<small>⚔️↓</small>":""}${x.shield?`<small>🔰${x.shield}</small>`:""}${(x.fx||[]).map(e=>({burn:"🔥",hot:"💚",stun:"💫",freeze:"🧊",slow:"🐌",vuln:"💔"})[e.k]||"").join("")}</div>`;
  root.innerHTML=`<div class="arena ${B.opp.horror?"horror falling":""} ${B.opp.johnny?"johnny":""} ${B.opp.bigrory?"bigrory":""} ${B.opp.johnny&&B.enemies.some(f=>f.id==="d157"&&f._phase2)?"phase2":""}">
...
function useSkill(actor,k,target,fromNet){
  ...
  const s=SKILLS[k];
  if(actor.id==="d163"&&actor.side==="ally"){charaFx(k,target);if(Math.random()<.35)setTimeout(charaSay,500)}
...
function applySkill(actor,k,target,atkM,blk){
  ...
  if(actor.id==="d157"){const ae=fEl(actor);const im=ae&&$("img",ae);if(im){im.src="assets/d157_raise.png";if(!actor._phase2)setTimeout(()=>{if(im)im.src="assets/d157.png"},900)}
  ...
  if(s.type==="attack"&&s.oneshot&&target){sndPlay("found",.9);const t=target;const el=fEl(t);if(el){el.classList.add("hit");vfx(el,"glitch")}
    ...
    else if(dInfo(t.id).boss){const dmg=Math.round(t.maxhp*0.2);t.hp=Math.max(0,t.hp-dmg);pop(el,"BAN −"+fmt(dmg),"crit");log(`🚫 <b>${t.name}</b> — БОСС: бан не берёт, снято 20% HP (${dmg})`)}
  ...
  if(s.type==="attack"){
    ...
    targets.forEach(t=>{const m=elMult(s.el,actor,t);const crit=RNG()<(actor.crit||0.1);const vuln=(t.fx||[]).some(e=>e.k==="vuln")?1.4:1;
      ...
      dmg=Math.max(1,Math.round(dmg));if(s.execute)dmg=Math.max(dmg,Math.round(t.maxhp*s.execute));if(t.hpScale&&t.hpScale!==1)dmg=Math.round(dmg*t.hpScale*(MOOSE_EVENT.dmgTaken||1));if(t.soviet&&t.side==="enemy"){const net=alive(B.enemies).filter(x=>x.soviet&&x!==t).length;dmg=Math.max(1,Math.round(dmg*(SOVIET_EVENT.dmgTaken||1)*(1-(SOVIET_EVENT.netShield||0)*net)));if(net)log(`🔗 сеть: урон по ${t.name} −${Math.round((SOVIET_EVENT.netShield||0)*net*100)}%`)}
      ...
      if(t.immortal&&t.hp-dmg<=0&&t.hp>1&&!(t._immR>=B.round-1)){dmg=t.hp-1;t._immR=B.round;pop(fEl(t),"БЕССМЕРТИЕ","heal");log(`🐂 ${t.name}: БЕССМЕРТИЕ — остаётся с 1 HP`)}
      t.hp=Math.max(0,t.hp-dmg);healed+=dmg;
      if(t.hp<=0&&t.soviet&&t.side==="enemy"&&!t._reboot&&alive(B.enemies).some(x=>x.soviet&&x!==t&&x.hp>0)){t._reboot=true;t.hp=Math.round(t.maxhp*(SOVIET_EVENT.reboot||0.35));t.charge=1;t.fx=[];pop(fEl(t),"REBOOT","crit");sndPlay("init",.9);log(`🖥️ <b>${t.name}</b>: АВАРИЙНАЯ ПЕРЕЗАГРУЗКА от сети — ${t.hp} HP, загрузка 1/3. Убей их всех в одном раунде или разорви сеть!`)}
    ...
    targets.forEach(t=>{const h=Math.round(t.maxhp*s.power*(1+((actor.dmgB||{})[s.el]||0))*(1+(actor.tb.healboost||0)));if((t.fx||[]).some(e=>e.k==="noheal")){pop(fEl(t),"ЛЕЧЕНИЕ ЗАПРЕЩЕНО","weak");const boss=alive(actor.side==="ally"?B.enemies:B.allies).find(x=>x.id==="d160");if(boss){boss.hp=Math.min(boss.maxhp,boss.hp+h);pop(fEl(boss),"+"+h,"heal");hellFx("helldrain",t);log(`🚫 ПРИГОВОР: лечение ${t.name} (${h}) ушло Лосю`)}else log(`🚫 ${t.name}: лечение заблокировано (ПРИГОВОР)`);return}t.hp=Math.min(t.maxhp,t.hp+h);
    ...
    alive(friends).forEach(t=>{if(s.effect==="shield"){t.shield=(t.shield||0)+Math.round(t.maxhp*s.power)}else if(k==="armor"){t.buffDef=1;t.buffDefT=3}else if(s.effect==="spd"){if(!t._spd0)t._spd0=t.spd;t.spd=Math.round(t._spd0*2.2);t.spdT=3;t.buffAtk=Math.max(t.buffAtk,s.power);t.buffAtkT=3;pop(fEl(t),"💨💨 x2.2","heal")}else{t.buffAtk=s.power;t.buffAtkT=3}vfx(fEl(t),"buff");pop(fEl(t),k==="armor"?"🛡️↑":"⚔️↑","heal")});SFX.buff();log(`${actor.name}: ${s.name} на всю команду!`);
...
function aiTurn(){
  ...
  let k=null,t=null;
  if(f.id==="d160"){f._mv=(f._mv||0)+1;if(f._hcR!==B.round){f._hcR=B.round;f.hcharge=(f.hcharge||0)+1}let k;if(B.round%3===0&&f._karaR!==B.round){f._karaR=B.round;k="hellkara"}else if(f.hcharge>=10&&f.hp<f.maxhp*.9&&f._hcU!==B.round){k="hellheal";f.hcharge=0;f._hcU=B.round}else{const ex=["hellfire","helldrain","hellgaze"].filter(x=>!(f.cds[x]>0));const pool=ex.length?ex.concat(RNG()<.25?["hellswap"]:[]):["hellswap"];k=pool[Math.floor(RNG()*pool.length)]}const t=(k==="hellfire"||k==="hellkara")?null:k==="hellheal"?f:foes.slice().sort((a,b)=>b.hp-a.hp)[0];if(k==="hellfire"){jumpscare(600)}hellFx(k,t);if(f._mv%3===0)hellSay();useSkill(f,k,t);return}
  if(f.id==="d164"){f._mv=(f._mv||0)+1;let k;if(f._mv%4===0)k="stampede";else if(f.hp<f.maxhp*.5&&!f._wall){f._wall=1;k="hornwall"}else if(f._mv%4===2)k="quake";else{const pool=["bullrush","roar"].filter(x=>!(f.cds[x]>0));k=pool[Math.floor(RNG()*pool.length)]||"bullrush"}const sk=SKILLS[k];const t=(sk.aoe||sk.type==="buff"||sk.type==="debuff")?null:foes.slice().sort((a,b)=>a.hp-b.hp)[0];roryFx(k,t);if(f._mv%3===0)rorySay();useSkill(f,k,t);return}
  ...
  if(f.soviet&&f.charge>=3){const tg=foes.slice().sort((a,b)=>b.hp-a.hp)[0];useSkill(f,"sovban",tg);return}
...
function dSkills(id){if(id==="d163")return ["knife","erase","determin","basic"];const o=S.dragons[id];const lvl=o?o.lvl:1;const d=dInfo(id);const out=baseSkills(id).filter((k,i)=>lvl>=SLOT_LEVEL[i]);
...
function openPhone(){
  ...
  $("#call",m).onclick=()=>{closeModal();pickTeamModal("📞 MR 333 отвечает…","<div class='desc'>«3:33. Ровно. Не опаздывай.» Команда 3:00 очень сильна — бери драконов, сильных против 🕒 3:00 (🔢 Цифра и 💉 Медицина) и берегись 🌙 Ночи.</div>",()=>{
    ...
    startBattle({name:"Команда 3:00 (MR 333)",lvl,ids:PHONE_EVENT.team,bg:BATTLE_BG.phone,onEnd:(win)=>{
      if(win){S.phoneWins=(S.phoneWins||0)+1;const r=PHONE_EVENT.reward;S.gold+=r.gold;S.gems+=r.gems;S.scrolls=(S.scrolls||0)+r.scrolls;addXP(200+40*lvl);let egg="";if(S.phoneWins%3===0){const got=S.dragons[PHONE_EVENT.egg]||(S.inv||[]).some(x=>x.id===PHONE_EVENT.egg)||S.phoneEggGot;const pool=got?PHONE_EVENT.pool:[PHONE_EVENT.egg];const eid=pool[Math.floor(Math.random()*pool.length)];S.phoneEggGot=true;giveEgg(eid,"event");egg="<p><b>🥚 MR 333 впечатлён — яйцо «"+dInfo(eid).name+"» в твоём инвентаре!</b></p>"}
...
function tleft(t){let s=Math.max(0,Math.floor((t-Date.now())/1000));const d=Math.floor(s/86400);s%=86400;const h=Math.floor(s/3600);s%=3600;const m=Math.floor(s/60);s%=60;return (d?d+(window.LANG==="en"?"d ":"д "):"")+String(h).padStart(2,"0")+":"+String(m).padStart(2,"0")+":"+String(s).padStart(2,"0")}
const QUESTS=[
...
function startSoviet(){if(S.level<SOVIET_EVENT.req){toast("Нужен уровень "+SOVIET_EVENT.req);return}closeModal();pickTeamModal("🖥️ Советские Компьютеры","<div class='desc'>init… init… ЦЕЛЬ НАЙДЕНА. Три машины в <b>сети</b>: пока живы соседи — урон по каждой сильно снижен (−30% за соседа). Каждая заряжает <b>BANNED</b> 3 хода — <b>оглушение/заморозка сбрасывает заряд</b>. Убитая машина <b>перезагружается</b> один раз, если сеть жива. Стратегия: контроль, фокус, добивание.</div>",()=>{
  const lvl=Math.max(S.level,SOVIET_EVENT.minLvl||30)+SOVIET_EVENT.lvlBonus;S.sovietLast=Date.now();save();
...
function johnnySay(p2){const a=$(".arena-field");if(!a||!B)return;const L1=["Посох выбирает достойного. Он выбрал МЕНЯ.","Лулу, не мешай. Я делаю это ради нас.","Портал шепчет: ещё один удар.","Ты видишь солнце? Оно смотрит на тебя.","Жархнне спит. Говорю Я."];const L2=["Я ГОРЮ — И ТЫ СГОРИШЬ СО МНОЙ!","СОЛНЦЕ НЕ ПАДАЕТ. СОЛНЦЕ ВОСХОДИТ.","ПОСОХ НЕ СЛОМАН. ПОСОХ ПРОСНУЛСЯ.","Больно… нет. НЕТ. ЭТО СИЛА.","Хранитель, СМОТРИ НА СВЕТ."];const L=p2?L2:L1;let d=$(".boss-say",a);if(!d){d=document.createElement("div");d.className="boss-say";a.append(d)}d.textContent="ВЕЛИКИЙ ДЖОННИ: "+L[Math.floor(Math.random()*L.length)];d.classList.toggle("p2",!!p2);d.classList.remove("on");void d.offsetWidth;d.classList.add("on")}
function johnnyPhase2Fx(){const a=$(".arena-field");if(!a)return;const o=document.createElement("div");o.className="jfx-rise";o.innerHTML="<div class='jfx-sun'></div><span>ВТОРАЯ ФАЗА</span><small>ПОСОХ ПРОСНУЛСЯ</small>";a.append(o);setTimeout(()=>o.remove(),2200);const ar=$(".arena");if(ar){ar.classList.add("hell-shake");setTimeout(()=>ar.classList.remove("hell-shake"),900)}try{SFX.charge()}catch(e){}}
function johnnySunFx(){const a=$(".arena-field");if(!a)return;const o=document.createElement("div");o.className="jfx-burst";o.innerHTML="<i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i>";a.append(o);setTimeout(()=>o.remove(),1400);setTimeout(()=>$$(".fighter.ally").forEach((e,i)=>setTimeout(()=>{const f=document.createElement("div");f.className="hfx hfx-flame";f.style.background="linear-gradient(0deg,#fc0 0%,#f80 30%,transparent 70%)";e.append(f);setTimeout(()=>f.remove(),1000)},i*100)),500)}
function startJohnny(){if(S.level<JOHNNY_EVENT.req){toast("Нужен уровень "+JOHNNY_EVENT.req);return}if(johnnyLeft()>0){toast("Джонни отдыхает: "+tleft(new Date(Date.now()+johnnyLeft())));return}closeModal();
  pickTeamModal("🕯️ Великий Джонни","<div class='desc'>Жархнне, усиленный порталом. Посох рассекает всех, удар оглушает. <b>Когда он падёт — он поднимет посох снова: вторая фаза, СОЛНЦЕ.</b> Каждая 2-я победа — 🥚 яйцо MR Lulu.</div>",()=>{
  const lvl=Math.max(S.level+JOHNNY_EVENT.lvlBonus,JOHNNY_EVENT.lateLvl);S.johnnyLast=Date.now();save();
  playDialog([["d157_hostage","Жархнне (в плену)","Хранитель… посох… он не мой… он держит меня…"],["d157_raise","Великий Джонни","Я — ВЕЛИКИЙ ДЖОННИ. Портал дал мне посох, посох дал мне ВСЁ."],["d135","MR Lulu","Джонни, опусти палку. Это же я, Лулу!"],["d136","MR Jarkhnne","Он не слышит. Портал говорит за него. Мы… мы будем драться на его стороне."],["d157","Великий Джонни","Палку?! ЭТО СОЛНЦЕ НА ПАЛКЕ. Смотри, как оно РАССЕКАЕТ."],["av0","Хранитель","Жархнне, держись. Я сломаю посох — даже если придётся сломать его дважды."]],()=>{
    startBattle({name:"Великий Джонни",lvl,ids:JOHNNY_EVENT.ids,bg:"citadel",boss:true,johnny:true,music:"johnny",onEnd:(win)=>{
...
function hellFx(k,t){const a=$(".arena-field");if(!a)return;const mk=(cls,ms,par)=>{const d=document.createElement("div");d.className="hfx "+cls;(par||a).append(d);setTimeout(()=>d.remove(),ms);return d};
  ...
  else if(k==="hellkara"){const d=mk("hfx-kara",1300);d.innerHTML="<span>КАРА</span>";const es=$$(".fighter.ally");if(es.length){setTimeout(()=>{es.forEach((e,i)=>setTimeout(()=>mk("hfx-hand",900,e),i*120));const a2=$(".arena");if(a2){a2.classList.add("hell-shake");setTimeout(()=>a2.classList.remove("hell-shake"),700)}},450)}}
...
/* ================= СЕКРЕТ: ПОДВАЛ БЫЧКОВ РОРИ ================= */
function charaSay(force){const a=$(".arena-field");if(!a||!B||!B.opp.chara)return;const L=B.opp.chara.mid||[];if(!L.length)return;let d=$(".boss-say.chara",a);if(!d){d=document.createElement("div");d.className="boss-say chara";a.append(d)}d.textContent=L[(B._csay=(B._csay||0)+1)%L.length];d.classList.remove("on");void d.offsetWidth;d.classList.add("on")}
function charaFx(k,t){const a=$(".arena-field");if(!a)return;const mk=(cls,ms,par)=>{const d=document.createElement("div");d.className="hfx "+cls;(par||a).append(d);setTimeout(()=>d.remove(),ms);return d};const ar=$(".arena");
  if(k==="knife"){const e=t&&fEl(t);if(!e)return;const d=mk("cfx-slash",700,e);d.innerHTML="<i></i><i></i><i></i>";setTimeout(()=>{mk("cfx-blood",900,e);const n=mk("cfx-9999",1100,e);n.textContent=Math.random()<.2?"9999":"";},260);if(ar){ar.classList.add("hell-shake");setTimeout(()=>ar.classList.remove("hell-shake"),250)}}
  else if(k==="erase"){const d=mk("cfx-erase",1400);d.innerHTML="<b>СТЕРЕТЬ</b>";document.body.classList.add("hell-glitch");setTimeout(()=>document.body.classList.remove("hell-glitch"),500);$$(".fighter.enemy").forEach((e,i)=>setTimeout(()=>{e.classList.add("erasing");setTimeout(()=>e.classList.remove("erasing"),900)},200+i*120))}
  else if(k==="determin"){const d=mk("cfx-det",1500);d.innerHTML="<span>❤</span><b>* Ты полна решимости.</b>";const e=fEl(B.allies[0]);if(e)mk("cfx-heart",1500,e)}
  else if(k==="basic"){const e=t&&fEl(t);if(e)mk("cfx-slash small",500,e)}}
function charaOffer(){const root=$("#modal-root");root.innerHTML=`<div class="chara-ov" id="cofr" style="cursor:default"><div style="font-family:monospace;color:#f00;font-size:22px;text-align:center;line-height:1.8;text-shadow:0 0 10px #f00"><div>* Ты проиграл. Снова.</div><div>* Но ты всё ещё здесь.</div><div style="margin-top:22px;font-size:26px"><span id="cyes" style="cursor:pointer">❤ ПРОДОЛЖИТЬ</span> &nbsp;&nbsp;&nbsp; <span id="cno" style="cursor:pointer;color:#888">СДАТЬСЯ</span></div></div></div>`;sndPlay("scream666",.15);
  $("#cno").onclick=()=>{root.innerHTML="";S.loseStreak=0;save();show("map")};
  $("#cyes").onclick=()=>{charaState().found=true;S.loseStreak=0;save();document.body.classList.add("hell-glitch");$("#cofr").innerHTML=`<div style="font-family:monospace;color:#f00;font-size:34px;text-shadow:0 0 14px #f00">* Приветик.</div>`;setTimeout(()=>{document.body.classList.remove("hell-glitch");root.innerHTML="";toast("<b style=\"color:#f00;font-family:monospace\">* Ты полон решимости. Смотри в Событиях.</b>");show("events")},1500)}}
function rorySay(){const a=$(".arena-field");if(!a||!B)return;const L=["МУУУУ!","ЗЕМЛЯ — МОЯ.","ТЫ СЛЫШИШЬ КОПЫТА?","ВСЕ БЫКИ — Я.","НЕ БЕГИ. ЭТО БЕСПОЛЕЗНО.","ПЫЛЬ. ПЫЛЬ. ПЫЛЬ."];let d=$(".boss-say",a);if(!d){d=document.createElement("div");d.className="boss-say rory";a.append(d)}d.textContent="БОЛЬШОЙ РОРИ: "+L[Math.floor(Math.random()*L.length)];d.classList.remove("on");void d.offsetWidth;d.classList.add("on")}
function roryFx(k,t){const a=$(".arena-field");if(!a)return;const mk=(cls,ms,par)=>{const d=document.createElement("div");d.className="hfx "+cls;(par||a).append(d);setTimeout(()=>d.remove(),ms);return d};const ar=$(".arena");const shake=ms=>{if(ar){ar.classList.add("hell-shake");setTimeout(()=>ar.classList.remove("hell-shake"),ms)}};
  if(k==="quake"){shake(900);mk("rfx-crack",1300);for(let i=0;i<10;i++){const r=mk("rfx-rock",1100);r.style.left=(5+Math.random()*90)+"%";r.style.animationDelay=(Math.random()*.4)+"s"}}
  else if(k==="stampede"){const d=mk("rfx-stamp",1500);d.innerHTML="<span>🐂</span><span>🐂</span><span>🐂</span><span>🐂</span><span>🐂</span>";shake(1400);setTimeout(()=>$$(".fighter.ally").forEach((e,i)=>setTimeout(()=>mk("rfx-dust",800,e),i*120)),600)}
  else if(k==="roar"){const d=mk("rfx-roar",1000);d.innerHTML="<b>МУУУ!</b>";shake(600)}
  else if(k==="hornwall"){const m=$(".fighter.enemy");if(m)mk("rfx-wall",1600,m)}
  else if(k==="bullrush"){const e=t&&fEl(t);if(e)setTimeout(()=>{mk("rfx-dust",800,e);shake(400)},300)}}
function roryState(){S.rory=S.rory||{floor:0,best:0,hp:null};return S.rory}
function roryFloorLvl(f){return RORY_EVENT.baseLvl+f*RORY_EVENT.step}
function openRory(){const g=roryState();if(g.floor>=RORY_EVENT.floors){modal(`<h2>🐂 Подвал Бычков Рори</h2><p>Подвал пройден до конца. Рекорд: ${g.best}/${RORY_EVENT.floors}.</p><div class="row"><button class="btn" id="rr">Начать заново</button></div>`);$("#rr").onclick=()=>{g.floor=0;g.hp=null;save();closeModal();openRory()};return}
  const f=g.floor,n=Math.min(3,1+Math.floor(f/3)),imm=f+1>=RORY_EVENT.immortalFrom;let lvl=roryFloorLvl(f);
  const last=f+1===RORY_EVENT.floors;if(last)lvl=RORY_EVENT.bossLvl;const m=modal(`<h2>🐂 ${last?"<span style='color:#f66'>ДНО ПОДВАЛА</span>":"Подвал Бычков Рори"} <span class="badge gold">этаж ${f+1}/${RORY_EVENT.floors}</span></h2><div class="feature"><img src="assets/${last?"d164":"d35"}.png" style="height:${last?160:120}px;filter:${last?"drop-shadow(0 0 18px #f30)":"brightness(.6) drop-shadow(0 0 12px #000)"}"><div style="flex:1"><div class="desc">${last?"Здесь стены дрожат. <b style='color:#f66'>БОЛЬШОЙ РОРИ</b>, уровень <b>"+lvl+"</b>. Землетрясения, стампида теней, рёв и стена рогов. Один. Ему хватит.":"Под островом что-то мычит. "+n+(n===1?" бычок":" бычка")+" уровня <b>"+lvl+"</b>."}${imm?" <b style='color:#f66'>БЕССМЕРТИЕ:</b> с этого этажа бычки не умирают от одного удара — их надо добивать.":""}<br><small>HP команды переносится между этажами. Поражение — на первый этаж.</small></div></div></div>
  ${g.hp?`<small>HP команды: ${Object.entries(g.hp).map(([id,h])=>dInfo(id).name+" "+h).join(", ")}</small>`:""}
  <div class="row"><button class="btn red" id="rgo">⬇ Спуститься</button>${g.hp?`<button class="btn green" id="rheal">💚 Лечить (💎${4+f})</button>`:""}${f>0?`<button class="btn" id="rexit">🚪 Выйти</button>`:""}</div><p><small>Рекорд: ${g.best}. Награда за этаж: 🪙${RORY_EVENT.rewardGold*(f+1)} 💎${RORY_EVENT.rewardGems}. За 10-й этаж — 🥚 Пол Маккартни.</small></p>`);
  $("#rgo",m).onclick=()=>{closeModal();pickTeamModal("🐂 Этаж "+(f+1),g.hp?"<p>⚠️ HP команды переносится.</p>":"",()=>{
    const fin=f+1===RORY_EVENT.floors;const go=()=>startBattle({name:fin?"БОЛЬШОЙ РОРИ":"Подвал Рори, этаж "+(f+1),lvl:fin?RORY_EVENT.bossLvl:lvl,bg:BATTLE_BG.dungeon,ids:fin?["d164"]:Array(n).fill("d35"),carry:g.hp||null,rory:true,bigrory:fin,immortal:imm&&!fin,boss:fin,music:fin?"gbt":undefined,onEnd:(win,hpLeft)=>{
      if(win){g.floor++;g.best=Math.max(g.best,g.floor);g.hp=hpLeft;const gold=RORY_EVENT.rewardGold*g.floor;S.gold+=gold;S.gems+=RORY_EVENT.rewardGems;S.scrolls=(S.scrolls||0)+2;addXP(120+40*g.floor);let extra="";if(g.floor===RORY_EVENT.floors){giveEgg(RORY_EVENT.finalEgg,"rory");extra="<p><b>🥚 Из подвала выходит… Пол Маккартни с чаем.</b></p>";g.hp=null}
        save();updateTop();modal(`<div class="result win">ЭТАЖ ${g.floor} ЗАЧИЩЕН</div><div class="center" style="font-weight:900">+🪙${fmt(gold)} +💎${RORY_EVENT.rewardGems} +📜2</div>${extra}<div class="row" style="justify-content:center"><button class="btn green" id="rn">Дальше</button></div>`,{closable:false});$("#rn").onclick=()=>{closeModal();show("events");openRory()}}
      else{g.floor=0;g.hp=null;save();modal(`<div class="result lose">${fin?"ЗАТОПТАН":"МУУУ."}</div><p class="center">Бычки затоптали. Рекорд: ${g.best}.</p><div class="row" style="justify-content:center"><button class="btn" onclick="closeModal();show('events')">Ок</button></div>`,{closable:false})}}});if(fin)playDialog([["d35","Бычок Рори","…му. Ты дошёл до самого низа."],["d164","БОЛЬШОЙ РОРИ","МУУУУУУУУ. Я — ПЕРВЫЙ БЫК. ВСЕ ОНИ — МОИ ТЕНИ."],["d164","БОЛЬШОЙ РОРИ","ЗЕМЛЯ ДРОЖИТ, КОГДА Я ИДУ. ТЫ — ПЫЛЬ ПОД КОПЫТОМ."],["av0","Хранитель","…он огромный. Держитесь!"]],go);else go()})};
  const rh=$("#rheal",m);if(rh)rh.onclick=()=>{const c=4+f;if(S.gems<c){toast("Не хватает 💎");return}S.gems-=c;g.hp=null;save();updateTop();openRory()};
  const re=$("#rexit",m);if(re)re.onclick=()=>{g.floor=0;g.hp=null;save();closeModal();toast("Вы вышли из подвала")}}
function roryCardHtml(){if(!S.roryFound)return "";const g=roryState();return `<div class="panel rory-card"><h2>🐂 СЕКРЕТ: Подвал Бычков Рори <span class="badge" style="background:#333">ур.${RORY_EVENT.req}+</span></h2><div class="feature"><img src="assets/d35.png" class="johnny-img" style="filter:brightness(.55)"><div style="flex:1"><div class="desc">10 этажей. Бычки с ${RORY_EVENT.baseLvl} уровня, +${RORY_EVENT.step} за этаж. С ${RORY_EVENT.immortalFrom}-го этажа — бессмертие. На 10-м — <b>БОЛЬШОЙ РОРИ</b>. А потом кто-то пьёт чай.</div><div class="row"><button class="btn" id="gorory" ${S.level<RORY_EVENT.req?"disabled":""}>⬇ Спуститься (этаж ${g.floor+1}, рекорд ${g.best})</button></div></div></div></div>`}

/* ================= СЕКРЕТ: МИССИИ ЧАРЫ ================= */
function charaState(){S.chara=S.chara||{done:{},found:false};return S.chara}
function charaCardHtml(){const c=charaState();if(!c.found)return "";const doneN=Object.keys(c.done).length;return `<div class="panel chara-card"><h2>🔪 ??? <span class="badge" style="background:#000">ур.15+</span></h2><div class="feature"><img src="assets/d163.png" class="johnny-img" style="image-rendering:pixelated"><div style="flex:1"><div class="desc">Кто-то другой хочет играть. Пройдено: ${doneN}/${CHARA_MISSIONS.length}.</div><div class="row">${CHARA_MISSIONS.map((m,i)=>`<button class="btn ${c.done[m.id]?"":"red"} sm" data-cm="${m.id}" ${i>0&&!c.done[CHARA_MISSIONS[i-1].id]?"disabled":""}>${c.done[m.id]?"✔ ":""}${m.name}</button>`).join("")}</div><small>Бои только за Чару. Игра выдаст её сама.${S.biscuitTwist?"<br><b style=\"color:#f66\">* Бисквит. Всё это время. Она оставила кольцо. Найди его: три цифры, которые ты уже видел.</b>":""}</small></div></div></div>`}
function charaMission(id){const m=CHARA_MISSIONS.find(x=>x.id===id);if(!m)return;closeModal();charaWalk(m,()=>{
  const lvl=m.lvl;
  const buff=window.__charaBuff||{};const go=()=>{try{startBattle({name:"Чара — "+m.name,lvl,ids:m.ids,bg:{island:"base",clinic:"room1",night:"darknet",hall:"gallery"}[m.room]||"nightmare0",boss:!!m.boss,allies:["d163"],allyOv:{lvl:Math.min(50,lvl+8),stars:5},music:"battle",chara:m,onEnd:(win)=>{
    if(win){const c=charaState();const first=!c.done[m.id];c.done[m.id]=1;const gold=2500*CHARA_MISSIONS.indexOf(m)+2500,gems=first?15:3;S.gold+=gold;S.gems+=gems;addXP(300);if(m.id==="c4"&&first){S.biscuitTwist=true;S.ring666=(S.ring666||0)}save();updateTop();
      const showRes=()=>modal(`<div class="result win" style="color:#f00;font-family:monospace">* ${m.win}</div><div class="center" style="font-weight:900">+🪙${fmt(gold)} +💎${gems}</div><div class="row" style="justify-content:center"><button class="btn" onclick="closeModal();show('events')">…</button></div>`,{closable:false});if(m.post&&first){if(m.id==="c4"){document.body.classList.add("hell-glitch");setTimeout(()=>document.body.classList.remove("hell-glitch"),600)}playDialog(m.post,showRes)}else showRes()}
    else{modal(`<div class="result lose" style="font-family:monospace">* Ты умерла. Но ты полна решимости.</div><div class="row" style="justify-content:center"><button class="btn" onclick="closeModal();show('events')">Попробовать снова</button></div>`,{closable:false})}}})}catch(err){console.error(err);B=null;modal(`<h2 style="color:#f00;font-family:monospace">* Ошибка</h2><p style="font-family:monospace">${String(err&&err.message||err)}</p><p><small>${String(err&&err.stack||"").split("\n").slice(0,3).join("<br>")}</small></p><div class="row"><button class="btn" onclick="closeModal();show('events')">Ок</button></div>`)}};if(m.pre&&!charaState().done[m.id])playDialog(m.pre,go);else go()})}
// мини-сцена: ходим Чарой по тёмной комнате (стрелки/WASD/свайп/джойстик), доходим до цели — начинается бой
function charaWalk(m,done){const root=$("#modal-root");const R=CHARA_ROOMS[m.room]||{items:[],deco:[]};root.innerHTML=`<div class="chara-ov"><div class="chara-room room-${m.room||"x"}" id="croom" style="background:${R.floor?R.floor+",":""}${R.bg||"#0a0a0a"}"><div class="chara-vig"></div>${(R.deco||[]).map((ic,i)=>`<div class="chara-deco" style="left:${8+i*19}%;top:${[8,78,12,70,40][i%5]}%">${ic}</div>`).join("")}${(R.items||[]).map((it,i)=>`<div class="chara-item" data-i="${i}" style="left:${it.x}%;top:${it.y}%">${it.ico}</div>`).join("")}<div class="chara-txt">* ${m.intro}</div><div class="chara-goal" id="cgoal">✦</div><div class="chara-pl" id="cpl"></div><div class="chara-hint">стрелки / WASD / свайп — дойди до ✦ &nbsp; <a id="cskip">пропустить</a></div><div class="cpad"><button data-d="up">▲</button><div><button data-d="left">◀</button><button data-d="down">▼</button><button data-d="right">▶</button></div></div></div></div>`;
  const room=$("#croom"),pl=$("#cpl"),goal=$("#cgoal");let x=40,y=60,dir=0,frame=0,moving=false;const keys={};const W=()=>room.clientWidth,H=()=>room.clientHeight;
  const G=R.goal||[80,45];goal.style.left=Math.min(W()-50,Math.max(200,W()*G[0]/100))+"px";goal.style.top=Math.min(H()-60,Math.max(80,H()*G[1]/100))+"px";const txt=$(".chara-txt",root);let txtT=null;const say=(t)=>{txt.innerHTML=t;txt.classList.remove("blink");void txt.offsetWidth;txt.classList.add("blink");clearTimeout(txtT);txtT=setTimeout(()=>{txt.textContent="* "+m.intro},4200)};window.__charaBuff={atk:0,heal:0};
  const items=$$(".chara-item",root);const checkItems=()=>{items.forEach(el=>{if(el.dataset.done)return;const ix=parseFloat(el.style.left)/100*W()+16,iy=parseFloat(el.style.top)/100*H()+16;if(Math.hypot(x+48-ix,y+60-iy)<58){el.dataset.done=1;el.classList.add("used");const it=R.items[+el.dataset.i];say(it.say);sndPlay("init",.4);if(it.atk)window.__charaBuff.atk+=0.15;if(it.heal)window.__charaBuff.heal=1}})};y=Math.max(0,Math.min(H()-96,parseFloat(goal.style.top)-44));
  const DIRS={down:0,left:1,right:2,up:3};let stop=false;
  const draw=()=>{pl.style.left=x+"px";pl.style.top=y+"px";pl.style.backgroundPosition=`-${frame*32*3}px -${dir*32*3}px`};
  const step=()=>{if(stop)return;let dx=0,dy=0;if(keys.left)dx-=1;if(keys.right)dx+=1;if(keys.up)dy-=1;if(keys.down)dy+=1;moving=dx||dy;if(moving){if(dy>0)dir=0;else if(dy<0)dir=3;if(dx<0)dir=1;else if(dx>0)dir=2;x=Math.max(0,Math.min(W()-96,x+dx*3));y=Math.max(0,Math.min(H()-96,y+dy*3));if(++tick%8===0)frame=(frame+1)%3;checkItems()}else frame=1;draw();
    const gx=parseFloat(goal.style.left)+16,gy=parseFloat(goal.style.top)+16;const px=x+48,py=y+60;if(Math.hypot(px-gx,py-gy)<70){stop=true;cleanup();root.innerHTML="";const g=document.createElement("div");g.className="hell-glitch";document.body.classList.add("hell-glitch");setTimeout(()=>{document.body.classList.remove("hell-glitch");done()},700);return}
    requestAnimationFrame(step)};let tick=0;
  const kd=e=>{const k={ArrowUp:"up",ArrowDown:"down",ArrowLeft:"left",ArrowRight:"right",w:"up",s:"down",a:"left",d:"right",ц:"up",ы:"down",ф:"left",в:"right"}[e.key];if(k){keys[k]=true;e.preventDefault()}};const ku=e=>{const k={ArrowUp:"up",ArrowDown:"down",ArrowLeft:"left",ArrowRight:"right",w:"up",s:"down",a:"left",d:"right",ц:"up",ы:"down",ф:"left",в:"right"}[e.key];if(k)keys[k]=false};
  const cleanup=()=>{window.removeEventListener("keydown",kd);window.removeEventListener("keyup",ku)};window.addEventListener("keydown",kd);window.addEventListener("keyup",ku);
  $$(".cpad button",root).forEach(b=>{const d=b.dataset.d;b.onpointerdown=e=>{e.preventDefault();keys[d]=true};b.onpointerup=b.onpointerleave=b.onpointercancel=()=>{keys[d]=false}});
  let sx=0,sy=0;room.onpointerdown=e=>{if(e.target.closest(".cpad")||e.target.id==="cskip")return;sx=e.clientX;sy=e.clientY};room.onpointermove=e=>{if(!sx&&!sy)return;const dx=e.clientX-sx,dy=e.clientY-sy;keys.left=dx<-12;keys.right=dx>12;keys.up=dy<-12;keys.down=dy>12};room.onpointerup=room.onpointercancel=()=>{sx=sy=0;keys.left=keys.right=keys.up=keys.down=false};
...
function playDialog(lines,done){if(!lines||!lines.length){done();return}let i=0;
  ...
  function draw(){let [img,who,text]=lines[i];if(window.t){who=window.t(who);text=window.t(text)}const isAv=img.startsWith("av"),far=img.startsWith("say_");root.innerHTML=`<div class="dlg-overlay"><div class="dlg"><img class="dlg-img ${far?"far":""}" src="assets/${isAv?"av/"+img+".jpg":img+".png"}"><div class="dlg-box"><div class="dlg-who">${who}</div><div class="dlg-text" id="dtext"></div><div class="dlg-hint">нажми, чтобы продолжить ▸ &nbsp; <a id="dskip">пропустить</a></div></div></div></div>`;
...
function renderFriends(){const root=$("#scr-friends");const on=NET.status==="online";const me=NET.me;
  ...
  $$("[data-av]",root).forEach(a=>a.onclick=()=>{if(a.dataset.price){const pr=+a.dataset.price;const av=AVATARS.find(x=>x.id===a.dataset.av);modal(`<h2>🐇 ${av.n}</h2><div class="center"><img src="assets/av/${av.id}.jpg" style="width:160px;border-radius:12px;border:3px solid #ffb700"><p>Секретная аватарка. Часы показывают 67.</p><p><b>💎${pr}</b></p></div><div class="row"><button class="btn purple" id="avbuy" ${S.gems>=pr?"":"disabled"}>Купить за 💎${pr}</button></div>`);$("#avbuy").onclick=()=>{S.gems-=pr;S.avOwned=(S.avOwned||[]).concat(av.id);S.profile.avatar=av.id;save();updateTop();closeModal();toast("🐇 67!");renderFriends()};return}S.profile.avatar=a.dataset.av;save();updateTop();if(NET.me)NET.me.avatar=S.profile.avatar;netSend({t:"profile",name:S.profile.name,avatar:S.profile.avatar,power:team.reduce((a,id)=>a+power(id),0),team});$$("[data-av]",root).forEach(x=>x.classList.toggle("sel",x.dataset.av===S.profile.avatar));if($("#scr-pvp").classList.contains("active"))renderPvp();else renderFriends()});
