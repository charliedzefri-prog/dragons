...
## Постоянное хранение базы (Render Free без диска)
Сервер хранит `db.json` в ветке `db` GitHub-репозитория. Переменные окружения:
- `GH_TOKEN` — токен с правом `contents: write` на репозиторий (обязательно, иначе база живёт только в контейнере и стирается при деплое)
- `GH_REPO` (по умолчанию `charliedzefri-prog/dragons`), `GH_BRANCH` (по умолчанию `db`)
